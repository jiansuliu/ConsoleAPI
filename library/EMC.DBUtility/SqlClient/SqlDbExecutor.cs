...
namespace EMC.DbUtility.SqlClient
{
  ...
  public class SqlDbExecutor : DbExecutorBase, IAsyncDbExecutor<ParameterizedQuery>, IDbTransactionProvider<SqlDbExecutor>
  {
    ...
    protected virtual IDbExecuteContext Execute( MySqlCommand command, IDbTracing tracing = null )
    {
    ...
    /// <param name="tracing">用于追踪查询过程的追踪器</param>
    /// <returns>查询执行上下文</returns>
    protected virtual async Task<IAsyncDbExecuteContext> ExecuteAsync( MySqlCommand command, CancellationToken token, IDbTracing tracing = null )
    {
      try
      {
        TryExecuteTracing( tracing, t => t.OnExecuting( command ) );
    ...
    protected MySqlCommand CreateCommand( ParameterizedQuery query )
    {
